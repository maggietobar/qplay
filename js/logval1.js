...
var errinputs = document.getElementsByClassName("input-group");
    ...
    validarMail: function () {
        if (mail.value != this.vacio) {
            if (this.mailreg.test(mail.value)) {
                errinputs[0].setAttribute("class","input-group");
                error[0].innerHTML = "";
                return true;
            } else {
                errinputs[0].setAttribute("class","input-group onerror");
               // form.mail.focus();
            ...
            if (pass.value.length >= this.minlargo) {
                if (this.passreg.test(pass.value)) {
                    if (pass.value != mail.value) {
                        errinputs[1].setAttribute("class","input-group");
                        error[1].innerHTML = "";
                        return true;
                    } else {
                        errinputs[1].setAttribute("class","input-group onerror");
                      //  form.pass.focus();
